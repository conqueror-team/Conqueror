...
import com.conqueror.blacklist.entity.blacklist.ColumnEntity;
import com.conqueror.blacklist.entity.blacklist.TableEntity;
...
public class GenUtils {
	...
	public static void generatorCode(Map<String, String> table,
		...
		//表名转换成Java类名
		String className = tableToJava(tableEntity.getTableName().toLowerCase(), config.getString("tablePrefix"));
		...
		for(Map<String, String> column : columns){
			...
			//在oracle里面获取的timestamp会有长度在后面所以用这种形式 Sven
			if(columnEntity.getDataType().toLowerCase().split("timestamp").length>1&&"unknowType".equals(attrType)){
				columnEntity.setAttrType("Date");
			}
			//是否主键
			if(("PRI".equalsIgnoreCase(column.get("columnKey"))||"P".equalsIgnoreCase(column.get("columnKey")))&& tableEntity.getPk() == null){
